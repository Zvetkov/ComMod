...
from pydantic import BaseModel, DirectoryPath, Field, FilePath, StringConstraints, ValidationError, computed_field, field_validator, model_validator

from commod.game.data import SupportedGames
from commod.helpers.file_ops import SUPPORTED_IMG_TYPES, get_internal_file_path, read_yaml
from commod.helpers.parse_ops import parse_simple_relative_path, process_markdown, remove_substrings
from commod.localisation.service import KnownLangFlags, SupportedLanguages, is_known_lang, tr
...
class Version(BaseModel):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return ((self.major.lower(), self.minor.lower(), self.patch.lower())
                ==
                (self.other.lower(), self.other.lower(), self.other.lower()))
...
class OptionalContent(BaseModel):
    ...
    def model_post_init(self, _unused_context: Any) -> None:  # noqa: ANN401
        if not self.data_dirs:
            if self.install_settings:
                self.data_dirs = [f"{self.name}/{setting.name}/data" for setting in self.install_settings]
            else:
...
class Mod(BaseModel):
    ...
    version: Version
    build: Annotated[str, StringConstraints(min_length=1, max_length=7, strip_whitespace=True)]
    ...
    @model_validator(mode="after")
    async def load_variants(self) -> "Mod":
        if self.is_variant and self.variants:
            raise ValueError("Mod variants can't have child variants")

        for variant_alias in self.variants:
            variant_manifest_path = Path(self.mod_files_root, f"manifest_{variant_alias}.yaml")
            ...
            mod_vr = Mod(**yaml_config, is_variant=True)
            self.add_mod_variant(variant_alias, mod_vr)

    @model_validator(mode="after")
    async def load_translations(self) -> "Mod":
        ...
        for translation_alias in self.translations:
            ...
            translation_manifest_path = Path(self.mod_files_root, manifest_name)
            ...
            self.add_mod_translation(translation_alias, mod_tr)
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        # loading default or backwards compat fallbacks
            ...
            if self.name == "community_patch":
                ...
                if self.mod_files_root.stem == "remaster":
                    self.mod_files_root = self.mod_files_root.parent
